...
import de.htwg.se.orderandchaos.data.control.controller.{Controller, TestController}
...
class TestControl(controllerToUse: Controller = new TestController, cell: Cell = Cell.red, returnValue: Try[Unit] = Success()) extends Control {
  ...
  override def finish(winner: String): Controller = controller
}
...
class ErrorControl(exception: Exception) extends Control {
  ...
  override def finish(winner: String): Controller = throw exception
}
